...
- **自訂設定**：覆寫 `nginx.conf`、`conf.d/default.conf`，並以 `entrypoint.bash` 在啟動時依環境變數替換監聽埠
...
- **步驟概覽**：
  1. **ghcr_login**：使用 `GITHUB_TOKEN` 登入 GHCR
  2. **nginx-image-build-amd64**：在 `ubuntu-24.04` 建置 AMD64 映像
  3. **nginx-image-build-arm64**：在 `ubuntu-24.04-arm` 建置 ARM64 映像
  4. **create-manifest**：建立並推送多架構 manifest `latest`
